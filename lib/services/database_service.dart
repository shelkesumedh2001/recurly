...
class DatabaseService {
  ...
  /// Register every Hive TypeAdapter the app uses. Idempotent. Split out
  /// so tests (which can't run Hive.initFlutter — it needs path_provider)
  /// can call `Hive.init(dir)` + this, then open boxes normally.
  static void registerAdapters() {
    ...
    // Credit-card due-date tracking adapter
    if (!Hive.isAdapterRegistered(9)) {
      Hive.registerAdapter(CreditCardInfoAdapter());
    }
  }

  /// Test-only: point the service at an already-open box (unit tests
  /// can't run [initialize] because Hive.initFlutter needs path_provider).
  @visibleForTesting
  // ignore: use_setters_to_change_properties
  void debugSetSubscriptionsBox(Box<Subscription> box) {
    _subscriptionsBox = box;
  }
  ...
  Future<void> initialize() async {
    try {
      ...
      // Offline sync write-queue (plain maps, no adapter needed)
      await Hive.openBox<dynamic>(AppConstants.syncQueueBox);
    } catch (e) {
